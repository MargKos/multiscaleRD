...
def functionsimulation(ts):
    ...
    for t in range(timesteps):
        ...
        if t%int(timesteps/timesteps_cut)==0 and t!=0:
            ...
            # save results
            np.save('/home/htc/bzfkostr/SCRATCH/SimulationsMultiscale/SIRSusTauBTest' + str(start) + 'time'
                    +str(k)+'', SusPositionHalfTime[k])
            np.save('/home/htc/bzfkostr/SCRATCH/SimulationsMultiscale/SIRInfTauBTest' + str(start) + 'time'
                    +str(k)+'',
                    InfPositionHalfTime[k])
            np.save('/home/htc/bzfkostr/SCRATCH/SimulationsMultiscale/SIRRecoveryTauBTest' + str(start) +'time'
                    +str(k)+ '',
...
np.save('/home/htc/bzfkostr/SCRATCH/SimulationsMultiscale/SIR_timeTauTestCutB'+str(start)+'.npy', elapsed_time)