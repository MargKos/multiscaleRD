...
# code for Exampel B in https://doi.org/10.1137/20M1352739

if len(sys.argv) < 2:
    print('No input provided...')
    sys.exit()
else:
    start = int(sys.argv[1]) - 1+2000
...
def functionsimulation(ts):
    ...
    for t in range(timesteps):
        ...
        if t%int((timesteps-1)/timesteps_cut)==0 and t!=0:
            ...
            np.save(f'/home/htc/bzfkostr/SCRATCH/SimulationsMultiscale/ExplicitProliferationParticles_{start}_time{k}.npy', PreyPosition)
...
np.save('/home/htc/bzfkostr/SCRATCH/SimulationsMultiscale/execution_timeExplicitProliferation'+str(start)+'.npy', elapsed_time)