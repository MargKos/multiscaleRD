...
all_sus_files = [
    np.load(f'/home/htc/bzfkostr/SCRATCH/SimulationsMultiscale/SIRSusTauBTest{s}time{time}.npy', allow_pickle=True)
    for s in range(500)
    for time in Times
]
all_inf_files = [
    np.load(f'/home/htc/bzfkostr/SCRATCH/SimulationsMultiscale/SIRInfTauBTest{s}time{time}.npy', allow_pickle=True)
    for s in range(500)
    for time in Times
]
all_rec_files = [
    np.load(f'/home/htc/bzfkostr/SCRATCH/SimulationsMultiscale/SIRRecoveryTauBTest{s}time{time}.npy', allow_pickle=True)
    for s in range(500)
    for time in Times
]
